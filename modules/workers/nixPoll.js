/*jshint esnext: true, moz: true, worker: true, node: true*/
'use strict';
...
function pollThis(fd) {
    ...
    if (cutils.jscEqual(length, -1)) {
      ...
      while (i < length) {
		...
		let len = casted.addressOfField('len').contents;
        let cookie = casted.addressOfField('cookie').contents;
        console.info('file_name:', fileName);
        console.info('type:', masks[mask]);
        i = ++len;
